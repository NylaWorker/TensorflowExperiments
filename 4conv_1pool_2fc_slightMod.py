...
cluster = tf.train.ClusterSpec({"ps": ["mist37-umh.cs.umn.edu:2225"],
                                "worker": ["mist37-umh.cs.umn.edu:5229"]})
...
if job_name == "ps":
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % task_number, cluster=cluster)):
        ...
        conv_layer1 = tf.nn.relu(
            conv2d(picture, W_conv1, stride=[1, 2, 2, 1]) + b_conv1)  ## chan you just add? don't you have to do np.add?
    ...
    with tf.train.MonitoredTrainingSession(master=server.target, is_chief=(task_number == 0)) as sess:
        ...
        # Train
	with open("Cost_output.csv", 'wb') as f:
