...
Reproducing some layers of the inception architechture
...
cluster = tf.train.ClusterSpec({"ps": ["mist31-umh.cs.umn.edu:2222"],
                                "worker": ["mist31-umh.cs.umn.edu:2222"]})
...
if job_name == "ps":
    ...
    with tf.device("/job:ps/task:0"):
        ...
        #
        # W_fc1 = weight_variable([256, 4096])  ## not sure about that 12*12*256
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % task_number, cluster=cluster)):
        ...
        conv_layer1 = tf.nn.relu(
            conv2d(picture, W_conv1, stride=[1, 2, 2, 1]) + b_conv1)  ## chan you just add? don't you have to do np.add?                                             bias=bias)
        print("Shape conv_layer1: ", conv_layer1.get_shape())

        pool_layer1 = max_pool(conv_layer1, k=[1, 3, 3, 1], pad='VALID')
        print("Shape pool_layer1: ", pool_layer1.get_shape())

        radius = 5; alpha = 0.0001;beta = 0.75;bias = 1.0
        normalized_layer1 = tf.nn.local_response_normalization(pool_layer1, depth_radius=radius, alpha=alpha, beta=beta,
                                                               bias=bias)
        print("Shape normalizde_layer1: ", normalized_layer1.get_shape())


        '''REALLY UNSURE ABOUT THIS ONE'''
        conv_layer2 = tf.nn.relu(
            conv2d(normalized_layer1, W_conv2) + b_conv2)  ## chan you just add? don't you have to do np.add?                                             bias=bias)
        print("Shape conv_layer2: ", conv_layer2.get_shape())

        conv_layer3 = tf.nn.relu(
            conv2d(conv_layer2,
                   W_conv3) + b_conv3)  ## chan you just add? don't you have to do np.add?                                             bias=bias)
        print("Shape conv_layer3: ", conv_layer3.get_shape())

        normalized_layer3 = tf.nn.local_response_normalization(conv_layer3, depth_radius=radius, alpha=alpha, beta=beta,
                                                               bias=bias)
        print("Shape normalized_layer3: ", normalized_layer3.get_shape())

        pool_layer3 = max_pool(normalized_layer3, pad='VALID')
        print("Shape pool_layer3: ", pool_layer3.get_shape())


        '''''''''''''''''''''''HERE IS WHERE INCEPTION STARTS'''''''''''''''''''''''
        ...
        # layer5 = max_pool(tf.nn.relu(conv2d(layer4, W_conv5) + b_conv5), k=[1, 2, 2, 1], pad='VALID')
