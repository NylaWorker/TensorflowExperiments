...
cluster = tf.train.ClusterSpec({"ps": ["mist31-umh.cs.umn.edu:2220"],
...
if job_name == "ps":
    ...
    with tf.device("/job:ps/task:0"):
        ...
        W_fc1 = weight_variable([4*4*256, 4096])## not sure about that 12*12*256
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % task_number, cluster=cluster)):
        ...
        layer1 = max_pool(normalized_layer1, k = [1, 1, 1, 1], pad='VALID')
        ...
        # layer2 = max_pool(normalized_layer2, k = [1, 1, 1, 1], pad = 'VALID')
        # print("Shape layer2 pooled: ", layer2.get_shape())
        layer3 = tf.nn.relu(conv2d(normalized_layer2, W_conv3) + b_conv3)
        ...
        fc2 = tf.nn.relu_layer(dropout_fc1, W_fc2, b_fc2)
